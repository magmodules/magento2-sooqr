...
class Category
{
    ...
    private function collectCategories(): array
    {
        ...
        $select = $this->resource->getConnection()
            ...
            ->where('catalog_category_entity.' . $this->linkField . ' <> ?', $this->getRootCategoryId());
        if ($this->excluded) {
        ...
        if ($this->includeAnchor) {
            ...
            $this->addAnchorCategories($path, $parentIds);
        }
        return $path;
    }
    ...
    private function addAnchorCategories(array &$path, array $parentIds): void
    {
        ...
        if ($this->excluded) {
            $select->where('catalog_category_entity.entity_id NOT IN (?)', $this->excluded);
        }
